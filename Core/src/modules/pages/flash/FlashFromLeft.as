package modules.pages.flash
{
	 ...
	 * 左侧飞入动画
	 */	
	public class FlashFromLeft implements IFlash
	{
		public function FlashFromLeft()
		{
		}
		...
		{
			return <flashFromLeft elementID={elementID} index={index}/>;
		}
		...
		public function next():void
		{
			...
			for each (var item:IElement in elements)
			{
				...
				p = item.canvas.stagePointToCanvas( - rect.width / 2, item.canvas.canvasPointToStage(item.x, item.y).y);
